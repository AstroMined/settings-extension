...
class SettingsOptions {
    ...
    renderAllSettings() {
        for (const [category, settingKeys] of Object.entries(this.settingCategories)) {
    ...
    async saveAllChanges() {
        ...
        try {
            ...
            this.updateSaveButton();
            this.showSuccess(`Successfully saved ${Object.keys(updates).length} setting(s)`);
    ...
    updateSaveButton() {
        ...
        const hasChanges = this.pendingChanges.size > 0 && this.validationErrors.size === 0;
    ...
    async resetToDefaults() {
        if (!confirm('Are you sure you want to reset all settings to defaults? This will discard any unsaved changes and cannot be undone.')) {
            return;
        }

...
describe("Content Script Integration", () => {
  ...
  describe("Settings API Exposure", () => {
    ...
    test("should handle getSetting API calls", async () => {
      ...
      expect(mockContentSettings.getSetting).toHaveBeenCalledWith(
        "testSetting",
      );
    ...
    test("should handle updateSetting API calls", async () => {
      ...
      expect(mockContentSettings.updateSetting).toHaveBeenCalledWith(
        "testSetting",
        false,
      );
    });
    ...
    test("should handle getSettings API calls", async () => {
      ...
      // Test the contentSettings instance directly since it's mocked
      const result = await mockContentSettings.getSettings([
        "setting1",
        "setting2",
      ]);

      expect(mockContentSettings.getSettings).toHaveBeenCalledWith([
        "setting1",
        "setting2",
      ]);
    ...
    test("should handle API call errors gracefully", async () => {
      ...
      // Test the contentSettings instance directly since it's mocked
      await expect(
        mockContentSettings.getSetting("testSetting"),
  ...
  describe("Message Handling", () => {
    ...
    test("should handle SETTING_CHANGED messages", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "Setting changed in content script:",
        mockEvent.key,
        mockEvent.value,
      );
    ...
    test("should handle SETTINGS_RESET messages", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "All settings reset in content script",
      );
      consoleSpy.mockRestore();
    });
  });

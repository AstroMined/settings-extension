...
describe('Message Passing Integration', () => {
  ...
  describe('End-to-End Message Flow', () => {
    ...
    test('should handle Content Script → Background → Storage → Broadcast flow', async () => {
      ...
      listeners.forEach(listener => {
        expect(listener).toHaveBeenCalledWith(
          broadcastMessage,
          expect.objectContaining({ tab: expect.objectContaining({ id: expect.any(Number) }) }),
          undefined
        );
      });
    });
    ...
    test('should handle export/import workflow', async () => {
      ...
      // Export settings
      const exportResponse = await mockRuntime.sendMessage({ type: 'EXPORT_SETTINGS' });
  ...
  describe('Error Handling', () => {
    test('should handle message delivery failures', async () => {
      ...
      mockRuntime.sendMessage.mockRejectedValue(new Error('Could not establish connection'));

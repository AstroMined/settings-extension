  ...
  const mockStorage = {
    ...
    getBytesInUse: jest.fn(async (keys = null) => {
      ...
      if (keys === null || keys === undefined) {
        ...
        targetStorage = storage[keys] !== undefined ? { [keys]: storage[keys] } : {};
      } else if (Array.isArray(keys)) {
        targetStorage = {};
        keys.forEach(key => {
          if (storage[key] !== undefined) {
    ...
    _setQuotaLimit: (limit) => { config.quotaLimit = limit; },
    _setErrorRate: (rate) => { config.errorRate = Math.max(0, Math.min(1, rate)); },
    _simulateQuotaExceeded: () => {
      ...
      return () => { mockStorage.set = originalSet; }; // Return cleanup function
    }
  };
...
function createMockRuntime(options = {}) {
  ...
  const mockRuntime = {
    ...
    _setBrowserType: (type) => { config.browserType = type; },
    _simulateError: (errorMessage) => {
      lastError = { message: errorMessage };
    },
    _clearError: () => { lastError = null; }
  };
...
async function testPerformance(testFunction, maxTime = 100, options = {}) {
  ...
  for (let i = 0; i < config.iterations; i++) {
    const memoryBefore = config.collectMemory && performance.memory ? 
  ...
  if (successfulRuns.length > 0) {
    ...
    if (config.collectMemory) {
      ...
      if (memoryDeltas.length > 0) {
        results.avgMemoryDelta = memoryDeltas.reduce((a, b) => a + b, 0) / memoryDeltas.length;
      }
    }
  } else {
...
function createMockBrowserEnvironment(browserType = 'chrome') {
  ...
  const mockEnvironment = {
    ...
    chrome: browserType === 'chrome' || browserType === 'edge' ? {
      storage: {
        local: mockStorage,
    ...
    browser: browserType === 'firefox' || browserType === 'edge' ? {
      storage: {
        local: mockStorage,
        sync: mockSyncStorage,
        onChanged: {

...
describe('SettingsManager', () => {
  ...
  describe('Initialization', () => {
    test('should initialize with default settings', async () => {
      ...
      expect(settingsManager.settings.size).toBe(Object.keys(testSettings).length);
      expect(settingsManager.settings.get('testBoolean')).toEqual(testSettings.testBoolean);
    });
  ...
  describe('CRUD Operations', () => {
    describe('Create/Set Settings', () => {
      ...
      test('should reject updates to non-existent settings', async () => {
        await expect(settingsManager.updateSetting('nonExistent', 'value'))
    ...
    describe('Read Settings', () => {
      ...
      test('should retrieve all settings', async () => {
        ...
        expect(Object.keys(allSettings)).toHaveLength(Object.keys(testSettings).length);
      ...
      test('should throw error for non-existent setting', async () => {
        await expect(settingsManager.getSetting('nonExistent'))
          .rejects.toThrow("Setting 'nonExistent' not found");
      });
    });
    ...
    describe('Update Settings', () => {
      ...
      test('should validate before updating', async () => {
        ...
        await expect(settingsManager.updateSetting('testBoolean', 'not a boolean'))
          .rejects.toThrow('must be a boolean');
          
        await expect(settingsManager.updateSetting('testNumber', 'not a number'))
    ...
    describe('Delete Settings', () => {
      ...
      test('should handle reset errors gracefully', async () => {
        ...
        await expect(settingsManager.resetToDefaults())
  ...
  describe('Validation', () => {
    ...
    test('should validate boolean settings', () => {
      ...
      expect(() => settingsManager.validateSetting(booleanSetting, true)).not.toThrow();
      expect(() => settingsManager.validateSetting(booleanSetting, false)).not.toThrow();
      expect(() => settingsManager.validateSetting(booleanSetting, 'true'))
        .toThrow('must be a boolean');
      expect(() => settingsManager.validateSetting(booleanSetting, 1))
        .toThrow('must be a boolean');
    });

    test('should validate text settings with length constraints', () => {
      const textSetting = { type: 'text', description: 'Test text', maxLength: 10 };
      
      expect(() => settingsManager.validateSetting(textSetting, 'short')).not.toThrow();
      expect(() => settingsManager.validateSetting(textSetting, '')).not.toThrow();
      expect(() => settingsManager.validateSetting(textSetting, 'this is too long'))
        .toThrow('exceeds maximum length of 10');
      expect(() => settingsManager.validateSetting(textSetting, 123))
        .toThrow('must be a string');
    });

    test('should validate longtext settings with length constraints', () => {
      const longTextSetting = { type: 'longtext', description: 'Test longtext', maxLength: 50 };
      
      expect(() => settingsManager.validateSetting(longTextSetting, 'acceptable length')).not.toThrow();
      expect(() => settingsManager.validateSetting(longTextSetting, 'x'.repeat(51)))
        .toThrow('exceeds maximum length of 50');
    });

    test('should validate number settings with min/max constraints', () => {
      const numberSetting = { type: 'number', description: 'Test number', min: 0, max: 100 };
      
      expect(() => settingsManager.validateSetting(numberSetting, 50)).not.toThrow();
      expect(() => settingsManager.validateSetting(numberSetting, 0)).not.toThrow();
      expect(() => settingsManager.validateSetting(numberSetting, 100)).not.toThrow();
      expect(() => settingsManager.validateSetting(numberSetting, -1))
        .toThrow('must be at least 0');
      expect(() => settingsManager.validateSetting(numberSetting, 101))
        .toThrow('must be at most 100');
      expect(() => settingsManager.validateSetting(numberSetting, 'not a number'))
        .toThrow('must be a valid number');
      expect(() => settingsManager.validateSetting(numberSetting, NaN))
        .toThrow('must be a valid number');
    });
    ...
    test('should validate JSON settings', () => {
      ...
      expect(() => settingsManager.validateSetting(jsonSetting, { key: 'value' })).not.toThrow();
      expect(() => settingsManager.validateSetting(jsonSetting, [])).not.toThrow();
      expect(() => settingsManager.validateSetting(jsonSetting, null))
        .toThrow('must be a valid object');
      expect(() => settingsManager.validateSetting(jsonSetting, 'string'))
        .toThrow('must be a valid object');
        
      // Test circular reference detection
      const circular = { self: null };
      circular.self = circular;
      expect(() => settingsManager.validateSetting(jsonSetting, circular))
    ...
    test('should reject invalid setting types', () => {
      ...
      expect(() => settingsManager.validateSetting(invalidSetting, 'value'))
  ...
  describe('Storage Integration', () => {
    ...
    test('should validate storage area selection', () => {
      expect(() => settingsManager.setStorageArea('invalid'))
        .toThrow('Storage area must be "local" or "sync"');
  ...
  describe('Import/Export', () => {
    ...
    test('should reject invalid JSON import', async () => {
      await expect(settingsManager.importSettings('invalid json'))
    ...
    test('should reject import without settings object', async () => {
      ...
      await expect(settingsManager.importSettings(JSON.stringify(invalidData)))
  ...
  describe('Error Handling', () => {
    test('should handle storage access errors', async () => {
      ...
      await expect(settingsManager.updateSetting('testBoolean', false))
        .rejects.toThrow('Permission denied');
    });

    test('should handle malformed settings data', async () => {
      const malformedSettings = {
        'badSetting': { /* missing type and value */ },
        'incompleteSetting': { type: 'boolean' /* missing value */ }
      };
      
      await expect(settingsManager.importSettings(JSON.stringify({ settings: malformedSettings })))
    ...
    test('should provide meaningful error messages', async () => {
      ...
      await expect(settingsManager.updateSetting('testNumber', 150))
        .rejects.toThrow('must be at most 100');
    });
  ...
  describe('Event System', () => {
    ...
    test('should emit events on setting changes', async () => {
      ...
      expect(listener).toHaveBeenCalledWith('updated', expect.objectContaining({
        key: 'testBoolean',
        value: false
      }));
    });
    ...
    test('should support event listeners', () => {
      ...
      expect(() => settingsManager.addListener('not a function'))

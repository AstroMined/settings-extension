...
describe('Storage Integration', () => {
  ...
  describe('Local Storage Operations', () => {
    ...
    test('should handle partial storage retrieval', async () => {
      await mockLocalStorage.set(testSettings);
      const partialResult = await mockLocalStorage.get(['testBoolean', 'testText']);
  ...
  describe('Sync Storage Operations', () => {
    test('should store and retrieve settings from sync storage', async () => {
      ...
      expect(retrieved.testJson.value).toEqual({ key: 'value', nested: { prop: 123 } });
    });
    ...
    test('should handle sync storage quota limits with realistic data sizes', async () => {
      ...
      await expect(mockSyncStorage.set(largeData)).rejects.toThrow('QUOTA_EXCEEDED_ERR');
    });
    ...
    test('should handle sync storage network errors gracefully', async () => {
      ...
      await expect(mockSyncStorage.set(testSettings)).rejects.toThrow('NetworkError');
    ...
    test('should fallback to local storage when sync is unavailable', async () => {
      ...
      mockSyncStorage.set.mockRejectedValue(new Error('Sync storage not available'));
  ...
  describe('Storage Quota Management', () => {
    test('should check local storage quota with real getBytesInUse', async () => {
      ...
      mockLocalStorage.getBytesInUse = jest.fn().mockResolvedValue(testDataSize);
    ...
    test('should handle quota exceeded scenario with realistic size calculations', async () => {
      ...
      for (let i = 0; i < 120; i++) {
        largeSettings[`largeSetting_${i}`] = { ...largeSetting, value: largeSetting.value + i };
      }
      ...
      await expect(mockLocalStorage.set(largeSettings)).rejects.toThrow('QuotaExceededError');
    });
  ...
  describe('Storage Persistence', () => {
    test('should persist settings across browser sessions using SettingsManager', async () => {
      ...
      const featureSetting = await newSettingsManager.getSetting('feature_enabled');
    ...
    test('should handle storage corruption recovery with embedded defaults', async () => {
      ...
      mockLocalStorage.get.mockRejectedValue(new Error('Storage corrupted: Invalid JSON'));
  ...
  describe('Storage Synchronization', () => {
    ...
    test('should handle sync conflicts with timestamp resolution', async () => {
      const localSettings = {
        feature_enabled: { type: 'boolean', value: false, description: 'Local version' },
        api_key: { type: 'text', value: 'local-key', description: 'API Key' }
      };
      
      const syncSettings = {
        feature_enabled: { type: 'boolean', value: true, description: 'Sync version' },
  ...
  describe('Storage Performance', () => {
    ...
    test('should handle concurrent storage operations without race conditions', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        const settingName = i % 2 === 0 ? 'refresh_interval' : 'feature_enabled';
        const newValue = i % 2 === 0 ? 30 + i : i % 2 === 1;
        concurrentUpdates[`${settingName}_${i}`] = { settingName, newValue };
        
        promises.push(
          settingsManager.updateSetting(settingName, newValue)
  ...
  describe('Storage Error Handling', () => {
    test('should handle Chrome lastError scenarios', async () => {
      ...
      mockLocalStorage.get.mockImplementation(() => {
        if (global.chrome.runtime.lastError) {
          return Promise.reject(new Error(global.chrome.runtime.lastError.message));
        }
        return Promise.resolve(mockLocalStorage.storage);
      });
      
      await expect(mockLocalStorage.get('testKey')).rejects.toThrow('The storage area is disabled.');
    ...
    test('should provide meaningful error messages with context', async () => {
      ...
      ).rejects.toThrow('Auto-refresh interval in seconds must be a valid number');
      
      await expect(
        settingsManager.updateSetting('refresh_interval', 5000) // Above max
      ).rejects.toThrow('Auto-refresh interval in seconds must be at most 3600');
  ...
  describe('Storage Security', () => {
    ...
    test('should handle sensitive data with proper validation', async () => {
      ...
      ).rejects.toThrow('API key for external service exceeds maximum length of 100');
    ...
    test('should prevent JSON injection attacks', async () => {
      ...
      ).rejects.toThrow('Advanced configuration object contains circular references or invalid JSON');

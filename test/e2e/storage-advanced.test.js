...
test.describe("Advanced Storage and Sync", () => {
  test.beforeAll(async (_, testInfo) => {
    ...
    const serviceWorker =
      await BrowserFactory.getExtensionServiceWorker(context);
  ...
  test.describe("Sync Conflicts and Resolution", () => {
    test("should handle concurrent setting updates correctly", async () => {
      ...
      try {
        ...
        await Promise.all([textInput1.fill(value1), textInput2.fill(value2)]);
        ...
        const finalValue1 = await page1
          .locator('input[type="text"]')
          .first()
          .inputValue();
        const finalValue2 = await page2
          .locator('input[type="text"]')
          .first()
          .inputValue();
  ...
  test.describe("Error Recovery and Resilience", () => {
    test("should recover from extension context invalidation", async () => {
      ...
      try {
        ...
        const recoveredValue = await page
          .locator('input[type="text"]')
          .first()
          .inputValue();

...
class ContentScriptSettings {
      ...
      try {
        browserAPI.runtime.sendMessage({
          ...
          if (response && response.error) {
            ...
            reject(new Error(`Invalid response for settings: ${keys.join(', ')}`));
          }
        }).catch((error) => {
          clearTimeout(timeout);
          reject(error);
      ...
      try {
        browserAPI.runtime.sendMessage({
          ...
          if (response && response.error) {
            ...
            reject(new Error(`Invalid response for updating setting '${key}'`));
          }
        }).catch((error) => {
          clearTimeout(timeout);
          reject(error);
  ...
  async updateSettings(updates) {
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`Timeout updating settings: ${Object.keys(updates).join(', ')}`));
      }, this.messageTimeout);
      ...
      try {
        browserAPI.runtime.sendMessage({
          ...
          if (response && response.error) {
            ...
            reject(new Error(`Invalid response for updating settings: ${Object.keys(updates).join(', ')}`));
          }
        }).catch((error) => {
          clearTimeout(timeout);
          reject(error);

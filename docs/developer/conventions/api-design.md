...
class EventEmittingService extends EventTarget {
  ...
  async startLongRunningOperation(operationId, data) {
    ...
    this.dispatchEvent(new CustomEvent('operationStart', {
    ...
    try {
      for (let i = 0; i < 100; i += 10) {
        ...
        this.dispatchEvent(new CustomEvent('operationProgress', {
...
const exportModule = await loader.loadModule('export', 
  () => import('./modules/export.js'));

const analyticsModule = await loader.loadModule('analytics',
  async () => {
...
describe('TestableService', () => {
  test('should cache fetched data', async () => {
    ...
    expect(mockStorage.set).toHaveBeenCalledWith('data:1', { id: 1, name: 'Test' });

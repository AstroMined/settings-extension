...
class SettingsMockFactory {
  static createMockSettings(options = {}) {
    const mockData = options.data || {
      feature_enabled: { type: 'boolean', value: true, description: 'Test feature' },
      api_key: { type: 'text', value: 'test-key-123', description: 'Test API key' },
      refresh_interval: { type: 'number', value: 60, description: 'Test interval', min: 1, max: 3600 }
    };
    ...
    switch (setting.type) {
      ...
      case 'longtext':
        ...
        if (setting.maxLength && value.length > setting.maxLength) {
          throw new Error(`Value exceeds maximum length of ${setting.maxLength}`);
        }
        break;
      case 'number':
...
class TestDataGenerator {
  static generateSettingsConfig(size = 'small') {
    const configs = {
      small: {
        feature_enabled: { type: 'boolean', value: true, description: 'Enable feature' },
        api_key: { type: 'text', value: 'test-key', description: 'API key' }
      },
      medium: {
        feature_enabled: { type: 'boolean', value: true, description: 'Enable feature' },
        api_key: { type: 'text', value: 'test-key-123456789', description: 'API key', maxLength: 100 },
        refresh_interval: { type: 'number', value: 60, description: 'Interval', min: 1, max: 3600 },
        custom_css: { type: 'longtext', value: '/* Test CSS */', description: 'CSS', maxLength: 10000 },
        advanced_config: { type: 'json', value: { endpoint: 'test' }, description: 'Config' }
      },
      large: {
  ...
  static generateInvalidValues() {
    return {
      ...
      string_too_long: { setting: { type: 'text', maxLength: 10 }, value: 'this string is way too long' },
...
class SettingsTestFramework {
  ...
  async run() {
    ...
    console.log(`  Passed: ${this.results.passed} (${Math.round(this.results.passed/total*100)}%)`);
    console.log(`  Failed: ${this.results.failed} (${Math.round(this.results.failed/total*100)}%)`);
    console.log(`  Skipped: ${this.results.skipped} (${Math.round(this.results.skipped/total*100)}%)`);
  ...
  static assertDeepEqual(actual, expected, message = '') {
    ...
    if (actualStr !== expectedStr) {
      throw new Error(`${message} Objects not deep equal.\nExpected:\n${expectedStr}\nActual:\n${actualStr}`);
    }
  }
}
...
function createUnitTestSuite() {
  ...
  framework.describe('Settings API Unit Tests', () => {
    ...
    framework.it('should get single setting', async () => {
      ...
      SettingsTestFramework.assertEqual(setting.type, 'boolean', 'Type should match');
      SettingsTestFramework.assertEqual(setting.value, true, 'Value should match');
      SettingsTestFramework.assertTrue(setting.description, 'Description should exist');
    });
    ...
    framework.it('should get multiple settings', async () => {
      const settings = await mockSettings.getSettings(['feature_enabled', 'api_key']);
      
      SettingsTestFramework.assertTrue(settings, 'Settings should exist');
      SettingsTestFramework.assertTrue(settings.feature_enabled, 'First setting should exist');
      SettingsTestFramework.assertTrue(settings.api_key, 'Second setting should exist');
      SettingsTestFramework.assertEqual(Object.keys(settings).length, 2, 'Should return 2 settings');
    });
    
    framework.it('should get all settings', async () => {
      const allSettings = await mockSettings.getAllSettings();
      
      SettingsTestFramework.assertTrue(allSettings, 'All settings should exist');
      SettingsTestFramework.assertTrue(Object.keys(allSettings).length >= 5, 'Should have multiple settings');
      SettingsTestFramework.assertTrue(allSettings.feature_enabled, 'Should contain feature toggle');
      SettingsTestFramework.assertTrue(allSettings.api_key, 'Should contain API key');
    });
    ...
    framework.it('should update single setting', async () => {
      ...
      const result = await mockSettings.updateSetting('feature_enabled', newValue);
      ...
      SettingsTestFramework.assertEqual(updatedValue.value, newValue, 'Value should be updated');
    });
    ...
    framework.it('should update multiple settings', async () => {
      ...
      const updatedSettings = await mockSettings.getSettings(['feature_enabled', 'refresh_interval']);
      SettingsTestFramework.assertEqual(updatedSettings.feature_enabled.value, false, 'Boolean should be updated');
      SettingsTestFramework.assertEqual(updatedSettings.refresh_interval.value, 120, 'Number should be updated');
    });
    ...
    framework.it('should export settings', async () => {
      ...
      SettingsTestFramework.assertTrue(parsed.timestamp, 'Should have timestamp');
      SettingsTestFramework.assertTrue(parsed.settings, 'Should have settings');
      SettingsTestFramework.assertTrue(Object.keys(parsed.settings).length > 0, 'Should have setting data');
    });
    ...
    framework.it('should import settings', async () => {
      const testData = {
        ...
        timestamp: new Date().toISOString(),
        settings: {
          feature_enabled: { type: 'boolean', value: false, description: 'Imported feature' },
          new_setting: { type: 'text', value: 'imported', description: 'New imported setting' }
        }
      };
      
      const result = await mockSettings.importSettings(JSON.stringify(testData));
      ...
      SettingsTestFramework.assertEqual(imported.value, false, 'Imported value should match');
      
      const newSetting = await mockSettings.getSetting('new_setting');
      SettingsTestFramework.assertEqual(newSetting.value, 'imported', 'New setting should exist');
    });
    ...
    framework.it('should reset to defaults', async () => {
      ...
      SettingsTestFramework.assertEqual(resetSetting.value, false, 'Should have default value');
    });
    ...
    framework.it('should cache settings after retrieval', async () => {
      ...
      SettingsTestFramework.assertFalse(cachedBefore, 'Should not be cached initially');
      ...
      SettingsTestFramework.assertTrue(cachedAfter, 'Should be cached after load');
      SettingsTestFramework.assertEqual(cachedAfter.type, 'boolean', 'Cached type should match');
    });
    
    framework.it('should clear cache', async () => {
      // Load and cache
      await mockSettings.getSetting('feature_enabled');
      SettingsTestFramework.assertTrue(mockSettings.getCachedSetting('feature_enabled'), 'Should be cached');
    ...
    framework.it('should notify change listeners', async () => {
      ...
      SettingsTestFramework.assertTrue(changeReceived, 'Change listener should be called');
      SettingsTestFramework.assertEqual(changeData.event, 'changed', 'Should receive changed event');
      SettingsTestFramework.assertEqual(changeData.data.feature_enabled, false, 'Should receive correct data');
    });
    ...
    framework.it('should remove change listeners', async () => {
      ...
      SettingsTestFramework.assertEqual(changeCount, 1, 'Should receive first change');
      
      mockSettings.removeChangeListener(listener);
      await mockSettings.updateSetting('feature_enabled', true);
      await new Promise(resolve => setTimeout(resolve, 10));
      
      SettingsTestFramework.assertEqual(changeCount, 1, 'Should not receive second change');
    });
    ...
    framework.it('should provide storage statistics', async () => {
      ...
      SettingsTestFramework.assertTrue(stats.totalBytes > 0, 'Should have total bytes');
      SettingsTestFramework.assertTrue(stats.settingsCount > 0, 'Should have settings count');
      SettingsTestFramework.assertTrue(stats.averageSettingSize > 0, 'Should have average size');
    });
    ...
    framework.it('should check storage quota', async () => {
      ...
      SettingsTestFramework.assertTrue(typeof quota.available === 'boolean', 'Should have availability');
      SettingsTestFramework.assertTrue(quota.used >= 0, 'Should have used bytes');
      SettingsTestFramework.assertTrue(quota.quota > 0, 'Should have quota limit');
      SettingsTestFramework.assertTrue(quota.percentUsed >= 0, 'Should have percentage');
    });
  });
...
function createIntegrationTestSuite() {
  ...
  framework.describe('Settings API Integration Tests', () => {
    ...
    framework.it('should handle complete user workflow', async () => {
      ...
      SettingsTestFramework.assertTrue(Object.keys(initialSettings).length > 0, 'Should load initial settings');
      ...
      for (const [key, expectedValue] of Object.entries(updates)) {
        SettingsTestFramework.assertEqual(modifiedSettings[key].value, expectedValue, `${key} should be updated`);
      }
      ...
      for (const [key, expectedValue] of Object.entries(updates)) {
        SettingsTestFramework.assertEqual(exported.settings[key].value, expectedValue, `${key} should be exported`);
      }
      ...
      SettingsTestFramework.assertNotEqual(resetSettings.setting_0.value, 'user_modified_0', 'Should be reset');
    });
    ...
    framework.it('should handle concurrent operations', async () => {
      ...
      SettingsTestFramework.assertTrue(results[0], 'First getSetting should complete');
      SettingsTestFramework.assertTrue(results[1], 'Second getSetting should complete');
      SettingsTestFramework.assertTrue(results[2], 'updateSetting should complete');
      SettingsTestFramework.assertTrue(results[3], 'getSettings should complete');
      SettingsTestFramework.assertTrue(results[4], 'getStorageStats should complete');
      
      // Verify update took effect
      const updated = await settings.getSetting('setting_2');
      SettingsTestFramework.assertEqual(updated.value, 'concurrent_update', 'Concurrent update should work');
    });
    ...
    framework.it('should handle large batch operations', async () => {
      ...
      SettingsTestFramework.assertTrue(updateTime < 1000, `Batch update should be fast, took ${updateTime}ms`);
      ...
      for (const key of keys) {
        SettingsTestFramework.assertEqual(updated[key].value, updates[key], `${key} should be batch updated`);
      }
    });
    
    framework.it('should maintain data consistency across operations', async () => {
      // Set initial value
      ...
      SettingsTestFramework.assertEqual(single.value, expectedValue, 'Single get should match');
      SettingsTestFramework.assertEqual(multiple.setting_0.value, expectedValue, 'Multiple get should match');
      SettingsTestFramework.assertEqual(all.setting_0.value, expectedValue, 'All get should match');
      SettingsTestFramework.assertEqual(cached.value, expectedValue, 'Cached should match');
    });
  });
...
function createPerformanceTestSuite() {
  ...
  framework.describe('Settings API Performance Tests', () => {
    ...
    framework.it('should load all settings within performance threshold', async () => {
      const startTime = performance.now();
      await settings.getAllSettings();
      const loadTime = performance.now() - startTime;
      
      // Should load within 100ms (adjust threshold as needed)
      SettingsTestFramework.assertTrue(loadTime < 100, `Settings load too slow: ${loadTime}ms`);
    });
    ...
    framework.it('should handle rapid sequential updates', async () => {
      ...
      console.log(`  📊 Rapid updates: ${updateCount} updates in ${Math.round(totalTime)}ms (${Math.round(avgTime)}ms avg)`);
      
      // Should average less than 10ms per update
      SettingsTestFramework.assertTrue(avgTime < 10, `Updates too slow: ${avgTime}ms average`);
    });
    ...
    framework.it('should efficiently use cache', async () => {
      ...
      console.log(`  📊 Cache performance: First load ${Math.round(firstLoadTime)}ms, Cached ${Math.round(cachedTime)}ms`);
      
      // Cached access should be much faster
      SettingsTestFramework.assertTrue(cachedTime < firstLoadTime / 10, 'Cached access should be 10x faster');
    });
    ...
    framework.it('should handle memory efficiently', async () => {
      ...
      console.log(`  📊 Memory usage: Growth ${Math.round(memoryGrowth / 1024)}KB`);
      
      // Should not grow memory excessively (adjust threshold as needed)
      SettingsTestFramework.assertTrue(memoryGrowth < 1024 * 1024, `Excessive memory growth: ${memoryGrowth} bytes`);
    });
  });
...
class SettingsTestRunner {
  ...
  generateReport() {
    ...
    console.log(`  Skipped: ${this.results.totalSkipped} (${Math.round(this.results.totalSkipped/this.results.totalTests*100)}%)`);
    ...
    for (const suite of this.results.suites) {
      ...
      console.log(`    Tests: ${total} | Passed: ${suite.passed} | Failed: ${suite.failed} | Success Rate: ${successRate}%`);
    }
...
console.log('🧪 Settings Testing Framework loaded. Access via window.SettingsTestingFramework');
console.log('   Run tests: window.SettingsTestingFramework.runAllTests()');
console.log('   Unit tests only: window.SettingsTestingFramework.runUnitTests()');
console.log('   Performance tests: window.SettingsTestingFramework.runPerformanceTests()');

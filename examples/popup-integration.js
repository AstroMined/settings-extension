...
class SimpleSettingsPopup {
   ...
   */
  renderSettings() {
    const container = document.getElementById('settings-list') || this.createSettingsContainer();
    container.innerHTML = '';
  ...
  createInputElement(key, setting) {
    ...
    switch (setting.type) {
      case 'boolean':
        ...
        input.addEventListener('change', () => this.updateSetting(key, input.checked));
        break;
      ...
      case 'text':
        ...
        input.addEventListener('blur', () => this.updateSetting(key, input.value));
        break;
  ...
  setSettingStatus(key, status) {
    const element = document.querySelector(`[data-key="${key}"] .setting-status`);
    if (element) {
      element.className = `setting-status ${status}`;
      element.textContent = status === 'updating' ? '...' : 
                           status === 'success' ? '✓' : 
                           status === 'error' ? '✗' : '';
